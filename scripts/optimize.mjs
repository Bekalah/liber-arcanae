// Image optimizer: AVIF/WEBP/JPEG pipeline with perceptual hash.
// ND-safe: runs offline; no motion or network requests.
...
for (const file of files) {
  ...
  const meta = await img.metadata();   // probe dimensions
  ...
  const phash = await imghash.hash(inPath); // perceptual hash
  entries.push({
    id: base,
